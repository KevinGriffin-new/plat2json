#!/usr/bin/env bash
# Wave 4: 40 more NCDOT ROW sheets (harvester skips already-harvested slugs),
# then read+score any unread ncdot_* sheet (same loop as 070, resumable).
source "$HOME/plat-queue/lib.sh"
"$PY" "$QDIR/ncdot_harvest.py" 40 || qlog "harvest exited nonzero (continuing to reads)"
