#!/usr/bin/env bash
# Wave 9: FULL-COVERAGE parcel run. Job 240's crops covered only golden-labeled
# chains (pilot honesty leak), so 63/441 planarized edges carried a course and
# no face could fully course. Production crops EVERYTHING: --crop-all, read
# every chain, planarize, close per face. Overnight-sized (~1-1.5 h total).
...
for slug in county_test adams_prc24_12 adams_prc2025; do
  ...
  qlog "fullcrop $vslug (page $page)"
  t0=$(date +%s)
  "$PY" "$EVAL/harness/assoc_study.py" "$PP/$slug.pdf" "$page" "$vslug" \
        --gt "$gt" --crop-all --workers 1 \
